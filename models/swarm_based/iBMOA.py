...
# FIXME
class iBMOA(OptimizationAlgorithm):
    ...
    def _apply_lsa_to_global_best__(self, current_FE, g_best=None):
        ...
        for ls in range(self.lsa_epoch):
            ...
            temp_best[self.ID_POS_BIN] = self._to_binary__(temp_best[self.ID_POS], g_best[self.ID_POS_BIN])
            for e in range(len(lsa_container)):
                if array_equal(lsa_container[e], temp_best[self.ID_POS_BIN]):
                    ...
                    if epoc >= self.epoch:
                        Terminate = True
                        break
                    lsa_container = concatenate((lsa_container, [temp_best[self.ID_POS_BIN]]), axis=0)
                    temp_best = self._to_binary_and_update_fit__(temp_best, temp_best[self.ID_POS])
